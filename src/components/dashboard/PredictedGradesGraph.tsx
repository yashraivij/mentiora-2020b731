          ...
          <div className="ml-16 pl-4 border-l border-b border-border/30 h-80 relative">
            ...
            <div className="h-full relative">
              {subjects.map((subject, index) => {
                const barWidth = Math.min(80, Math.max(48, (100 / subjects.length) - 16));
                const totalWidth = subjects.length * (barWidth + 16);
                const startX = Math.max(0, (100 - totalWidth) / 2);
                const xPosition = startX + (index * (barWidth + 16));
                
                return (
                  <div key={subject.id} className="absolute bottom-0 group" style={{ left: `${xPosition}%` }}>
                    {/* Floating grade & emoji */}
                    <div className="absolute -top-20 left-1/2 transform -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-all duration-300 -translate-y-2 group-hover:translate-y-0">
                    ...
                    {/* Animated bar */}
                    <div 
                      className="rounded-t-lg relative overflow-hidden shadow-lg group-hover:shadow-2xl transition-all duration-500 group-hover:scale-105"
                      style={{ 
                        width: `${barWidth}px`,
                    ...
                    <div className="absolute -bottom-16 left-1/2 transform -translate-x-1/2 text-center">
                      <div className="text-xs font-medium text-foreground truncate" style={{ maxWidth: `${barWidth}px` }}>
                        {subject.name}
                      </div>
