...
interface PremiumPredictedGradesGraphProps {
  userProgress: UserProgress[];
}

export const PremiumPredictedGradesGraphUnblurred = ({ userProgress }: PremiumPredictedGradesGraphProps) => {
  console.log('🚀 PREMIUM GRADES COMPLETELY UNBLURRED - FORCED RERENDER 🚀');
  const { user } = useAuth();
                        ...
                        <div className={`relative h-40 bg-gradient-to-t from-gray-100/30 to-gray-50/20 dark:from-gray-800/30 dark:to-gray-700/20 rounded-3xl overflow-hidden border-2 border-white/20 shadow-xl ${getSubjectShadow(index)} group-hover:shadow-2xl transition-all duration-500`}>
                          {grade.finalGrade !== '–' && (
                            <>
                              ...
                              {/* Premium glow overlay - percentage filled with minimum 8% for 0% cases */}
                              <div 
                                className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-white/20 to-transparent rounded-3xl opacity-60"
                                style={{ height: `${Math.max(grade.finalPercentage, 8)}%` }}
                              />
                              
                              {/* Animated shimmer effect - percentage filled with minimum 8% for 0% cases */}
                              <div 
