    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[900px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          >
            <h1 className="text-3xl font-bold text-gray-900 mb-2">One plan. All features.</h1>
        ...
        <div className="px-2">
          <motion.p 
            ...
            transition={{ duration: 0.5 }}
          >
            Pay just once to get all-in-one access until {currentPricing.endDate}, or subscribe monthly.
          </motion.p>
          ...
          {!showParentForm ? (
            <>
              ...
              <AnimatePresence mode="wait">
                ...
                >
                  {/* Monthly */}
                  <Card className="border border-gray-200 rounded-xl p-6 bg-white shadow-sm hover:shadow-md transition-shadow">
                    <div className="flex items-start justify-between mb-4">
                      <div>
                        <h3 className="text-lg font-semibold mb-1 text-gray-900">7-Day Free Trial</h3>
                        <p className="text-gray-600 text-xs">Start free, then £9.99/month.</p>
                      </div>
                      <div className="text-right">
                        <div className="flex items-baseline gap-1">
                          <span className="text-3xl font-bold text-gray-900">£0</span>
                          <span className="text-gray-600 text-xs">/week</span>
                        </div>
                        <p className="text-xs text-gray-500 mt-1">Then £9.99/month</p>
                      </div>
                    </div>
                    ...
                    <Button
                      ...
                      variant="outline"
                      className="w-full h-11 rounded-lg text-sm font-semibold border-2 border-gray-900 hover:bg-gray-50"
                    >
                      Start Free Trial
                    </Button>
                  ...
                  {/* Exam Access */}
                  <Card className="border-l-4 border-l-[#00A8FF] border border-gray-200 rounded-xl p-6 bg-white shadow-sm hover:shadow-md transition-shadow relative">
                    <div className="absolute top-4 right-4 bg-[#00A8FF] text-white px-2 py-1 rounded text-xs font-semibold">
                      Save 60%
                    </div>
                    
                    <div className="flex items-start justify-between mb-4">
                      <div>
                        <h3 className="text-lg font-semibold mb-1 text-gray-900">{examYear} Exam Access</h3>
                        <p className="text-gray-600 text-xs">
                          One-time payment. Access until {currentPricing.endDate}.
                        </p>
                      </div>
                      <div className="text-right">
                        <div className="flex items-baseline gap-1 mb-1">
                          <span className="text-3xl font-bold text-gray-900">£{currentPricing.oneTimePrice}</span>
                          <span className="text-gray-600 text-xs">once</span>
                        </div>
                        <span className="text-gray-400 line-through text-xs">£{currentPricing.originalPrice}</span>
                      </div>
                    </div>

                    <Button
                      onClick={handleOneTimePayment}
                      className="w-full h-11 rounded-lg text-sm font-semibold bg-[#00A8FF] hover:bg-[#0096E6] text-white"
                    >
                      Get Exam Access
                    </Button>

                    <p className="text-xs text-gray-500 mt-2 text-center">
                      £{currentPricing.oneTimePrice} billed once.
                    </p>
                  </Card>
                </motion.div>
              </AnimatePresence>
              ...
              {/* Comparison Line */}
              <motion.div 
                className="text-center text-xs text-gray-600 mb-6 pb-6 border-b border-gray-200"
                initial={{ opacity: 0 }}
