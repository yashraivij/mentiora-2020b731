...
import { BarChart3, BookOpen, TrendingUp, User, LogOut, Flame, Calendar, CheckCircle, Trophy, Filter, Star, Pin, Crown, Zap, Brain, Target, Clock, LineChart, Sparkles, Bell, Gamepad2, Settings } from "lucide-react";
...
const PremiumDashboard = () => {
  ...
  useEffect(() => {
    ...
    if (urlParams.get('payment') === 'success') {
      ...
      // Clean up the URL
      window.history.replaceState({}, document.title, '/premium-dashboard');
    }
  }, [toast]);
      ...
      if (data) {
        ...
        const subjectIds = data.map(record => {
          ...
          const subject = curriculum.find(s => 
            ...
            s.name === record.subject_name
          );
          
          const matchedId = subject ? subject.id : null;
          console.log(`=== Matched to curriculum ID: ${matchedId} ===`);
          return matchedId;
        }).filter(id => id !== null) as string[];

        console.log('=== Final converted subject IDs ===', subjectIds);
  ...
  const handlePhysicsToggle = async (subjectId: string) => {
    console.log(`=== Physics Toggle Called: ${subjectId} ===`);
    console.log('User subjects before:', userSubjects);
    
    // Find the subject in curriculum
    const subject = curriculum.find(s => s.id === subjectId);
    if (!subject) {
      console.error('Subject not found in curriculum');
      return;
    }
    ...
    try {
      if (userSubjects.includes(subjectId)) {
        // Remove subject
        console.log('=== Removing Physics subject ===');
        ...
        if (error) {
          console.error('Database error removing subject:', error);
          toast({
            title: "Error",
        ...
        // Add subject
        console.log('=== Adding Physics subject ===');
  ...
  // Handle toggling subjects in/out of user's list
  ...
  const handleNotificationAction = () => {
    if (notification.type === "weak-topic-recommendation" && notification.subjectId) {
      navigate(`/subjects/${notification.subjectId}`);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-background via-background to-muted/10">
      ...
      <div className="container mx-auto px-6 py-8 max-w-7xl">
            ...
            <CardContent className="relative p-6">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                    ...
                    {/* Compact streak display */}
                     <div className="space-y-2">
                       ...
                       <div className="flex items-center justify-center space-x-2">
                         ...
                         >
                           {getStudyStreak() === 1 ? 'day streak' : 'day streak'}
                         </motion.span>
                         <motion.div
                           initial={{ scale: 0 }}
                           animate={{ scale: 1 }}
                  ...
                  {/* Compact streaks rewards */}
                  <div className="space-y-2.5">
                    <div className="flex items-center justify-between p-3 bg-white/30 dark:bg-white/5 rounded-xl backdrop-blur-sm border border-white/20">
                      <div className="flex items-center space-x-3">
                    ...
                    <div className="flex items-center justify-between p-3 bg-white/30 dark:bg-white/5 rounded-xl backdrop-blur-sm border border-white/20">
                      ...
                      <div className="flex items-center space-x-2">
                        <span className="text-xs text-muted-foreground">Study Playlist</span>
                        {getStudyStreak() >= 7 ? (
                          <CheckCircle className="h-4 w-4 text-emerald-500" />
                        ) : (
                          <div className="w-4 h-4 rounded-full border-2 border-muted-foreground/30" />
                    ...
                    <div className="flex items-center justify-between p-3 bg-white/30 dark:bg-white/5 rounded-xl backdrop-blur-sm border border-white/20">
                      <div className="flex items-center space-x-3">
                        ...
                        <span className="text-sm font-medium text-foreground">14 Day Streak</span>
                      </div>
                      <div className="flex items-center space-x-2">
                        <span className="text-xs text-muted-foreground">Free Tutoring</span>
                        {getStudyStreak() >= 14 ? (
                          <CheckCircle className="h-4 w-4 text-emerald-500" />
                        ) : (
                          <div className="w-4 h-4 rounded-full border-2 border-muted-foreground/30" />
                ...
                {/* RIGHT SIDE: Public Streak Profiles */}
                <div className="space-y-4">
        ...
        {/* Premium Predicted Questions Section - FULLY UNLOCKED */}
        <div className="mb-6">
        ...
        {/* Premium Analytics Cards - ALL UNLOCKED */}
        ...
        {/* Advanced Analytics Unlocked Section */}
          ...
          <div className="flex flex-col items-center space-y-6">
            ...
            <div>
              ...
              <p className="text-muted-foreground max-w-lg mx-auto text-lg">
                All premium analytics features are now available to supercharge your GCSE revision. Enjoy the full experience!
              </p>
            </div>
            <Button 
              onClick={() => navigate('/analytics')}
              className="mt-6 px-8 py-3 text-lg font-semibold bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-700 hover:to-blue-700 text-white transition-all duration-300"
            >
