...
import { mathQuestions } from "@/data/diagnosticQuestions";

export const MathDiagnostic = () => {
  ...
  const handleNext = () => {
    if (currentQuestion < mathQuestions.length - 1) {
      ...
      // Save results and navigate to transition
      localStorage.setItem('diagnostic_math_answers', JSON.stringify(answers));
      localStorage.setItem('diagnostic_math_time', timeElapsed.toString());
      navigate('/diagnostic/transition');
    }
  };
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="h-[72px] border-b border-[#E1E8ED] px-8 flex items-center justify-between">
        <div className="flex items-center gap-4">
          <span className="text-[18px] font-bold text-[#0A1628]">
            Math Diagnostic | Question {currentQuestion + 1} of {mathQuestions.length}
          </span>
        </div>
        <div className="flex items-center gap-4">
      ...
      <div className="max-w-[900px] mx-auto px-20 py-12">
        ...
        <div className="flex items-center justify-between max-w-[700px]">
          ...
          >
            {currentQuestion === mathQuestions.length - 1 ? 'Complete Math →' : 'Next Question →'}
          </button>
        </div>

        {/* Progress Indicator */}
        <div className="mt-8 text-center text-[14px] text-[#8899A6]">
          {answeredCount} of {mathQuestions.length} answered
        </div>
      </div>
    </div>
  );
