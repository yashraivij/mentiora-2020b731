...
const PredictedQuestions = () => {
  ...
  const { subjects, isLoading: curriculumLoading } = useCurriculum();
          ...
          <TabsList className="grid w-full grid-cols-5 max-w-md mx-auto bg-card/80 backdrop-blur-xl border border-border/50 rounded-2xl p-1 shadow-2xl dark:bg-card/90 dark:border-border/70">
           ...
           <TabsContent value="aqa" className="mt-6">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
               {/* Special handling for AQA Computer Science - show 2 separate papers */}
           ...
           {['edexcel', 'ccea', 'ocr', 'wjec'].map((examBoard) => (
            <TabsContent key={examBoard} value={examBoard} className="mt-6">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                 {/* Special handling for OCR Computer Science - show 2 separate papers */}
                 {examBoard === 'ocr' && (
                   <>
                     {/* Computer Science Paper 1 - J277/01 */}
                     <Card 
                  ...
                  {subjects
                    .filter((subject) => {
                      ...
                      // For OCR tab, exclude computer-science since we handle it specially above
                      if (examBoard === 'ocr' && subject.id === 'computer-science') {
                        return false;
                      }
