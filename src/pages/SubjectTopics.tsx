      ...
      <div className="max-w-7xl mx-auto px-8 py-10">
        <div className="grid lg:grid-cols-[2fr,1fr] gap-8">
          ...
          <div className="space-y-8">
            ...
            <Card className="rounded-2xl border-border shadow-lg dark:shadow-xl overflow-hidden relative">
              ...
              <CardContent className="pt-8 pb-8 relative z-10">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-3">
                    <div className="w-12 h-12 rounded-full bg-gradient-to-br from-primary to-primary/70 flex items-center justify-center animate-pulse">
                      <Rocket className="h-6 w-6 text-primary-foreground" />
                    </div>
                    <div>
                      <h3 className="text-xl font-semibold text-foreground">Overall Mastery</h3>
                      <p className="text-sm text-muted-foreground">Keep going, you're making great progress!</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="text-3xl font-bold bg-gradient-to-r from-primary to-primary/70 bg-clip-text text-transparent">
                      {Math.round(topicMastery.reduce((acc, t) => acc + t.mastery, 0) / topicMastery.length)}%
                    </div>
                ...
                <div className="relative h-4 bg-secondary rounded-full overflow-hidden shadow-inner">
                  ...
                  <div 
                    ...
                    style={{ 
                      width: `${topicMastery.reduce((acc, t) => acc + t.mastery, 0) / topicMastery.length}%`,
                  ...
                  {/* Glowing dots along progress */}
                  {[20, 40, 60, 80].map((milestone) => {
                    const currentProgress = topicMastery.reduce((acc, t) => acc + t.mastery, 0) / topicMastery.length;
                    ...
                    return (
                      <div
                        ...
                        className={`absolute top-1/2 -translate-y-1/2 w-2 h-2 rounded-full transition-all duration-500 ${
                          isPassed ? 'bg-primary-foreground scale-125 shadow-lg' : 'bg-muted-foreground/30 scale-100'
                        }`}
                        style={{ 
                          left: `${milestone}%`,
                          boxShadow: isPassed ? '0 0 10px hsl(var(--primary))' : 'none',
                ...
                <div className="flex justify-between mt-2 px-1">
                  {['Beginner', 'Learning', 'Progressing', 'Advanced', 'Master'].map((label, idx) => {
                    const milestone = idx * 25;
                    const currentProgress = topicMastery.reduce((acc, t) => acc + t.mastery, 0) / topicMastery.length;
                    ...
                    return (
                      <div 
                        ...
                        className={`text-xs transition-all duration-500 ${
                          isReached ? 'text-primary font-semibold scale-110' : 'text-muted-foreground'
                        }`}
                      >
                        {label}
                      </div>
