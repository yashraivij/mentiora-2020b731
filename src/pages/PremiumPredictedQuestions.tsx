...
const PremiumPredictedQuestions = () => {
  ...
  const handleSubjectSelect = (subjectId: string) => {
    // Direct navigation to exam without paywall
    navigate(`/premium-predicted-exam/${subjectId}`);
  };
              ...
              {isCompleted ? (
                <>
                  <Button 
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      navigate(`/premium-predicted-results/${subject.id}`, { 
                        state: { 
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                 {curriculum
                   .filter((subject) => {
                     ...
                     // For CCEA, OCR, WJEC tabs, show AQA subjects as "Coming Soon"
                     return true;
                   })
                   .map((subject) => renderSubjectCard(subject, examBoard))}
              </div>
            </TabsContent>
          ))}
        </Tabs>
...
export default PremiumPredictedQuestions;
