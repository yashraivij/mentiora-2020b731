...
const PredictedResults = () => {
  ...
  // Save exam completion to database when results are calculated - with retry logic
  useEffect(() => {
    const saveExamCompletion = async (retryCount = 0) => {
      if (attempts.length > 0 && !isReview) { // Only save if not a review session
        try {
          const { data: { user } } = await supabase.auth.getUser();
          if (!user) {
            console.error('❌ No user found for saving exam completion');
            return;
          }

          console.log('💾 Saving exam completion to database (attempt ' + (retryCount + 1) + '):', {
          ...
          if (error) {
            console.error('❌ Error saving exam completion (attempt ' + (retryCount + 1) + '):', error);
            
            // Retry up to 3 times with exponential backoff
            if (retryCount < 3) {
              const delay = Math.pow(2, retryCount) * 1000; // 1s, 2s, 4s
              console.log(`🔄 Retrying in ${delay}ms...`);
              setTimeout(() => saveExamCompletion(retryCount + 1), delay);
            } else {
              console.error('❌ Failed to save exam completion after 3 attempts');
              toast.error("Failed to save exam results after multiple attempts. Your results are still displayed.");
            }
          } else {
            console.log('✅ Exam completion saved successfully!');
            toast.success("Exam results saved! Review/Retry buttons will appear shortly.");
          }
        } catch (error) {
          console.error('❌ Unexpected error in saveExamCompletion (attempt ' + (retryCount + 1) + '):', error);
          
          // Retry on unexpected errors too
          if (retryCount < 3) {
            const delay = Math.pow(2, retryCount) * 1000;
            console.log(`🔄 Retrying after unexpected error in ${delay}ms...`);
            setTimeout(() => saveExamCompletion(retryCount + 1), delay);
          } else {
            console.error('❌ Failed to save exam completion after 3 attempts due to unexpected errors');
            toast.error("Failed to save exam results. Please try refreshing the page.");
          }
        }
      }
    };
